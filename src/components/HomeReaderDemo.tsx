  ...
  return (
    ...
    >
      ...
      <div className="grid items-center" style={{ gridTemplateColumns: '1fr auto 1fr', padding: '10px 16px', borderBottom: '1px solid var(--border-subtle)' }}>
        ...
        <span className="inline-flex items-center justify-self-center" style={{ gap: 8, height: 32, padding: '0 13px', borderRadius: 'var(--radius-md)', border: '1px solid var(--neutral-200)', boxShadow: 'var(--shadow-e1)', fontSize: 12.5, color: 'var(--text-muted)' }}>
      ...
      <div className="flex items-stretch">
        ...
        <aside className="hidden md:flex flex-col flex-shrink-0" style={{ width: 184, borderRight: '1px solid var(--border-subtle)', background: 'var(--surface-main)' }} aria-hidden>
          ...
          <ul style={{ listStyle: 'none', margin: 0, padding: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
            {SURAHS.map((s) => {
              const active = s.n === 1;
              return (
                <li key={s.n} className="flex items-center" style={{ gap: 10, padding: '8px 10px', borderRadius: 'var(--radius-md)', background: active ? 'var(--green-soft)' : 'transparent' }}>
                  <span className="inline-flex items-center justify-center flex-shrink-0" style={{ width: 24, height: 24, borderRadius: 8, fontSize: 11, fontWeight: 700, background: active ? 'var(--green-600)' : 'var(--neutral-100)', color: active ? 'var(--accent-contrast)' : 'var(--text-muted)' }}>{localizeDigits(s.n, locale)}</span>
        ...
        <div className="flex flex-col items-center flex-1 min-w-0" style={{ background: 'var(--surface-app)', padding: 'clamp(12px, 2.5vw, 22px)' }}>
          ...
          <div className="flex items-center overflow-x-auto thin-scroll" style={{ gap: 3, padding: 7, borderRadius: 'var(--radius-lg)', background: 'var(--surface-main)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--shadow-e2)', maxWidth: '100%', pointerEvents: 'none' }} aria-hidden>
            {ALL_TOOLS.map((t) => {
              const active = t === ACTIVE_TOOL;
              return (
                <span key={t} className="flex flex-col items-center justify-center shrink-0"
                  style={{ gap: 3, width: 50, height: 48, borderRadius: 'var(--radius-md)', background: active ? 'var(--green-soft)' : 'transparent', color: active ? 'var(--text-accent)' : 'var(--text-muted)' }}>
          ...
          <div className="relative mt-4" style={{ width: 'fit-content', maxWidth: '100%', borderRadius: 'var(--radius-page)', overflow: 'hidden', boxShadow: 'var(--shadow-e2)' }}>
            ...
            <div className="absolute inset-0" aria-hidden style={{ pointerEvents: 'none' }}>
              ...
              <span style={{ position: 'absolute', left: '80%', top: '58%', width: 20, height: 20, border: '2.5px solid var(--home-marker)', borderRadius: '50%' }} />
              ...
              <svg style={{ position: 'absolute', left: '8%', top: '38%', width: 18, height: 18 }} viewBox="0 0 24 24" fill="none" stroke="var(--home-note-green)" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"><path d="M4 12l5 5L20 6" /></svg>
            </div>
          </div>
        </div>
        ...
        <aside className="hidden md:flex flex-col flex-shrink-0" style={{ width: 208, borderLeft: '1px solid var(--border-subtle)', background: 'var(--surface-main)' }} aria-hidden>
          ...
          <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {NOTE_KEYS.map((nt, i) => (
              <div key={i} className="flex items-start" style={{ gap: 9, padding: '10px 11px', borderRadius: 'var(--radius-md)', background: 'var(--surface-app)', border: '1px solid var(--border-subtle)' }}>
