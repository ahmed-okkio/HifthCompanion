...
export default function SetsList({ initialSets }: { initialSets: AnnotationSet[] }) {
  ...
  async function handleDeleteSet(id: string) {
    if (!confirm(t('sets.confirmDelete'))) return;
    try {
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="card flex gap-2 items-center"
        ...
        <button onClick={handleCreateSet}
          ...
          <span className="sm:inline">{t('common.create')}</span>
        </button>
      </div>
      ...
      {sets.length === 0 && (
        <div className="card text-center animate-fade-in"
          ...
          <p className="text-sm font-medium">{t('sets.noneYetTitle')}</p>
          <p className="text-xs mt-1">{t('sets.noneYetHint')}</p>
        </div>
      )}
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="sets-actions flex gap-1 flex-shrink-0">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t('sets.rename')}</span>
                </button>
                {!set.is_default && (
