...
24. As a reader, I want changing pages (prev/next, jump, surah select) to swap only the page image and its annotations and notes in place — updating the surah panel active state when needed — instead of reloading the whole reader, so that navigation is instant and my context is never torn down. _(Model: Opus)_
25. As a maintainer, I want the canvas-draw E2E tests (pen, highlighter, undo, redo, share read-only canvas, persistence) to pass reliably instead of flaking, so that a green suite actually means the reader works and agents stop wasting cycles re-running to classify flakes. _(Model: Sonnet)_

## Implementation Decisions

### Story 24 — Soft page swap (added after Story 23)
- **Goal:** page navigation within the reader becomes a shallow in-place update, not a full route remount/server round-trip. The reader shell, surah panel, toolbar, and the Fabric canvas instance stay mounted; only the page image source, the loaded `canvas_json` for the new (set, page), and the notes for the new page are swapped. The surah panel's active button updates to reflect the new page.
- **Behavior to preserve (regression-sensitive):** active annotation set is kept (`?set=`), surah-panel scroll position is kept, same-page surahs stay combined, share view is unaffected, and the existing data-testid hooks + E2E guards stay green. Clicking the current page's surah must still be a no-op (no churn).
- **Approach (implementer decides exact mechanism):** drive the current page from client state synced to the URL via shallow navigation, so the `[page]` segment updates without tearing down the canvas; reuse the existing per-page load path (image + `canvas_json` + notes) to repopulate the mounted canvas (clear objects, load new state) rather than disposing/reinitialising it. Must flush/save the outgoing page's canvas before swapping (existing `__hifthFlushReaderCanvas` hook). No data model, API, or schema changes.
